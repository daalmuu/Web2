...
if (!isset($_FILES['photo']) || $_FILES['photo']['error'] !== 0) {
    ...
    if ($check === false) {
        $errors[] = "Invalid photo. Please upload a real image.";
    } else {
...
if (isset($_FILES['video']) && $_FILES['video']['error'] === 0 && $_FILES['video']['name'] !== '') {
    ...
    if ($is_image !== false) {
        ...
        if (move_uploaded_file($tmp_video, $upload_dir . $video_name)) {
...
try {
    ...
    if ($video_path && file_exists($upload_dir . $video_path)) unlink($upload_dir . $video_path);
